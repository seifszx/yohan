...
async def start_hunter_game(game_id, context, chat_id, msg_id, players):
    ...
    spots_pool = list(HIDING_SPOTS)
    random.shuffle(spots_pool)
    for i, player in enumerate(normal_players):
        # كل لاعب يحصل على 3 أماكن مختلفة للاختيار
        player_spots = random.sample(spots_pool, min(3, len(spots_pool)))
        hunter_games[chat_id]['hiding_spots'][player['id']] = {
            'options':  player_spots,
            'chosen':   None,
        }
        buttons = [
            [InlineKeyboardButton(f"🏠 {spot[:30]}...", callback_data=f"hide_{chat_id}_{player['id']}_{j}")]
        ...
        try:
            await context.bot.send_message(
                ...
                text=(
                    ...
                    f"اختر أحد الأماكن التالية للاختباء فيه:\n"
            ...
            logger.warning(f"send hiding spots to {player['first_name']}: {e}")

    await asyncio.sleep(1)

    # ═══ إرسال رسالة خاصة للمطارد ═══
    player_buttons = [
        [InlineKeyboardButton(f"👤 {p['first_name']}", callback_data=f"huntpick_{chat_id}_{p['id']}")]
    ...
    try:
        ...
        logger.warning(f"send hunter message: {e}")
...
async def hunter_guess_spots(chat_id, context, hunter_id, target_player_id):
    ...
    buttons = [
        [InlineKeyboardButton(f"🔍 {spot[:35]}", callback_data=f"huntguess_{chat_id}_{target_player_id}_{j}")]
        for j, spot in enumerate(spots)
    ]
    buttons.append([InlineKeyboardButton("🔙 رجوع", callback_data=f"huntback_{chat_id}")])
        ...
        if guessed_spot == correct_spot:
            ...
            if not remaining:
                ...
                for pid in hg['hiding_spots']:
                    ...
                    new_spots = random.sample(HIDING_SPOTS, min(3, len(HIDING_SPOTS)))
                    hg['hiding_spots'][pid]['options'] = new_spots
                ...
                for player in hg['players']:
                    spot_data_new = hg['hiding_spots'][player['id']]
                    player_spots = spot_data_new['options']
                    buttons = [
                        [InlineKeyboardButton(f"🏠 {spot[:30]}...", callback_data=f"hide_{hg_chat_id}_{player['id']}_{j}")]
                        for j, spot in enumerate(player_spots)
                    ]
                    try:
                        await context.bot.send_message(
                ...
                player_buttons = [
                    [InlineKeyboardButton(f"👤 {p['first_name']}", callback_data=f"huntpick_{hg_chat_id}_{p['id']}")]
                    for p in hg['players']
                ]
                try:
                    await context.bot.send_message(
                ...
                player_buttons = [
                    [InlineKeyboardButton(
                        ...
                        callback_data=f"huntpick_{hg_chat_id}_{p['id']}"
                    )]
                    for p in hg['players']
                ]
